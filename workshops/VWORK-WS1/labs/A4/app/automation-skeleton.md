...
> Confirm the scheduled-task dialog fields, the pre-authorize-read-only control, and the Studio-workflow-in-Work path against the live product as you go. The frequency options (once / daily / weekly / monthly / yearly) and the approval controls are grounded from the prior live capture; confirm the pre-authorize control's on-screen label.
...
Confirm the Studio-workflow-in-Work entry point and step configuration in-app.
