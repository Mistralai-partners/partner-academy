...
2. **Prove the guardrail.** In a live session, ask Vibe to draft and **send** a heads-up message to a safe test destination. When the approval prompt appears, **Reject** it, and confirm nothing ran.
...
Never approve a send to a real recipient during this lab. You will Reject first.

## Self-contained note

A4 needs only a safe test write destination. It uses no Library, Skill, connector, or output from another lab. See `governance-posture-card.md` for the rule and `automation-skeleton.md` for the build.
