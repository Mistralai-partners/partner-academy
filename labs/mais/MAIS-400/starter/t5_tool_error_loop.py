#!/usr/bin/env python
"""Task 5 (STARTER) - Resilient function-result error loops.

Your job: make the tool loop survive a lookup miss. When `get_inventory` is
called with an unknown SKU it must return a STRUCTURED error result that goes
back into the conversation, so the model can recover and produce a final answer.
...
def get_inventory(sku):
    # SYMPTOM: an unknown SKU raises and crashes the tool loop instead of letting the model recover. See tasks.md (Task 5).
