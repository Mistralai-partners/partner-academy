#!/usr/bin/env python
"""Task 2 (STARTER) - Give an agent a tool and return its result.

Behavior (maps to MAIS-200 B1, FKC Q3): when a conversation asks for data the
model cannot know, the model emits a `function.call` output. You must run the real
function and feed the answer back with a `FunctionResultEntry` whose `tool_call_id`
MATCHES the call the model made. Without that, the model never gets its result and
the conversation cannot finish.
...
Source: platform-docs-public public/studio-api/agents/handoffs.md +
        conversations/function-calling.md (pinned).
"""
import json
import os
import sys
...
def resolve(client):
    ...
    # TODO: the model has asked to call get_order_status but has NOT received a
    # result, so the conversation is stuck at a function.call and never produces a
    # final answer. Execute the requested function and return its result to the
    # conversation so the model can finish. The result must be tied to the exact
    # call the model made. Right now we just hand back the unfinished response:
    return resp
