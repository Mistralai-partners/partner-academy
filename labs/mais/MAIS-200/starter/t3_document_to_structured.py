#!/usr/bin/env python
"""Task 3 (STARTER) - Turn a document into structured data with Document AI.
...
but is not exposed in mistralai==1.9.11, so it is taught only, not called here.
...
def extract(client):
    ...
    TODO: this call runs plain OCR - it returns page text but NO structured
    annotation, so `document_annotation` comes back empty and nothing downstream
    can rely on typed fields. Make the OCR return data shaped as DocMeta so that
    `response.document_annotation` parses into that model (see main()).
    """
