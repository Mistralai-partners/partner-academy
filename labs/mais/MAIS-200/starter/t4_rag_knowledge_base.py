#!/usr/bin/env python
"""Task 4 (STARTER) - Build a small RAG knowledge base.

Behavior (maps to MAIS-200 B3, FKC Q7/Q8): the from-scratch RAG order is
chunk -> embed (`mistral-embed`) -> retrieve by similarity -> ground the answer in
what you retrieved, and refuse when the fact is absent. Retrieval must rank by
...
def retrieve(query_vec, corpus_vecs):
    """Return (best_index, best_score) for the passage that best answers the query.

    TODO: this returns the FIRST chunk every time, ignoring the query, so the
    grounded answer is built on the wrong passage and the refuse-when-absent check
    never triggers. Rank the corpus against the query and return the best match
    (and its score) by direction, not by document order.
    """
    return 0, cosine(query_vec, corpus_vecs[0])
