# A3: Reference Outcome (solution)

What success looks like. Your tool and item differ; the shape is the same.

## Featured OAuth connector (primary path)

- On the Connectors page, the chosen tool shows **connected**, with its **per-function permissions** visible and set to read-only. `[VERIFY]` the exact permission labels at capture.
- In the task, you asked for one **named** item ("Summarize Q3-plan.pdf from my Drive and list its sections").
- Vibe Work prompted for approval on the **read** action; you approved it.
